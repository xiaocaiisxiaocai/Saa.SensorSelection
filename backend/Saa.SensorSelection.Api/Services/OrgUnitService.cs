...
public class OrgUnitService(AppDbContext db)
{
        ...
        CancellationToken ct = default)
    {
        if (request.ParentId is int parentId &&
            !await db.OrgUnits.AnyAsync(org => org.Id == parentId, ct))
        {
        ...
        CancellationToken ct = default)
    {
        ...
        if (request.ParentId is int parentId)
        {
            ...
            if (!await db.OrgUnits.AnyAsync(item => item.Id == parentId, ct))
            {
                return RbacResult<OrgUnitListItem>.Fail("父级组织不存在");
            }
        ...
        org.Level = string.IsNullOrWhiteSpace(request.Level) ? null : request.Level.Trim();
