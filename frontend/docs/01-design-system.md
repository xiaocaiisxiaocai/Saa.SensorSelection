...
> **2026-09 补充**：实施过程中发现表格/筛选类密集界面在这版字号下仍偏松，之后又做了一轮"紧凑桌面密度"调整——把 display/title/headline/body 各降一档，并新增 `--text-field`（表单控件）、`--text-toolbar`（筛选工具栏）两档更小的专用尺寸。这是**在这份文档写完之后、且已经落地并被 `tokens.test.ts` 保护**的有意决策，下表是**当前实际生效**的值，不再是最初的推荐值；上一段"对照依据"描述的是最初设计意图，仅作背景说明。

| 令牌 | 字号/行高 | 字重 | 字距 | 用途 |
| --- | --- | --- | --- | --- |
| `--text-display` | 28 / 36 | 700 | -0.5px | 登录页主标题 |
| `--text-title-1` | 24 / 30 | 600 | -0.4px | 页面标题 |
| `--text-title-2` | 20 / 26 | 600 | -0.3px | 区块标题、弹窗标题 |
| `--text-headline` | 15 / 22 | 600 | -0.2px | 列表项主文字、强调 |
| `--text-body` | 15 / 22 | 400 | 0 | 正文、说明 |
| `--text-control` | 15 / 22 | 400 | 0 | **默认**：按钮、输入框、表格单元格 |
| `--text-control-em` | 15 / 22 | 600 | 0 | 表头、tag |
| `--text-field` | 13 / 18 | 400 | 0 | 表单控件内文字（输入框、下拉、菜单项） |
| `--text-field-em` | 13 / 18 | 600 | 0 | 表单控件内加粗文字 |
| `--text-toolbar` | 12 / 18 | 400 | 0 | 筛选工具栏内控件文字（更紧凑的密度） |
| `--text-toolbar-em` | 12 / 18 | 600 | 0 | 筛选工具栏内加粗文字 |
...
- 负字距只加在 15px 及以上（原规则"17px 以上"随 headline/body 降到 15px 一并下调）；13px 及以下一律用 0，避免中文挤在一起。
...
固定布局尺寸（随 3.2 节同一轮紧凑密度调整一起改过，当前实际值）：

| 令牌 | 值 | 说明 |
| --- | --- | --- |
| `--sidebar-width` | 152px | 主导航侧栏（展开态，不可调） |
| `--sidebar-collapsed-width` | 56px | 主导航侧栏折叠态 |
| `--source-list-width` | 220px | 页面内实体侧栏（可拖拽） |
| `--toolbar-height` | 52px | 顶部工具栏 |
| `--row-height` | 36px | 表格行（紧凑） |
| `--row-height-loose` | 44px | 带头像/多行内容的行 |

粗指针（触屏）设备下，`@media (pointer: coarse)` 会把 `--row-height`/`--row-height-loose`/`--control-height-*` 整体调回更接近原推荐的触控尺寸（详见 `tokens.css`），桌面鼠标场景才用上表的紧凑值。
...
桌面鼠标场景下的实际值（同一轮紧凑密度调整过，见 3.2 节说明）：

| 尺寸 | 高度 | 横向内边距 | 字号 | 圆角 |
| --- | --- | --- | --- | --- |
| `small` | 26px | 8px | 13px | 6px |
| `medium`（默认） | 30px | 10px | 15px | 8px |
| `large` | 34px | 12px | 15px | 8px |
| `xlarge` | 42px | 16px | 15px | 10px |

`@media (pointer: coarse)`（触屏）下回到更接近原推荐的触控尺寸：`small` 36、`medium` 40、`large`/`xlarge` 44–48，具体值见 `tokens.css`。

- 图标按钮：正方形，尺寸与上表同名档位对齐，圆角同上或 `--radius-pill`。
- **触控目标最小 44×44**：视觉尺寸可以更小（如桌面下 `medium` 30px），但可点击区域用透明内边距撑到 44px（`::before` 扩展命中区），粗指针媒体查询下控件本身也会直接放大到该尺寸。
- 表格里的图标按钮例外，允许 26–28px 命中区（桌面鼠标场景，且行高本身较紧凑）。
