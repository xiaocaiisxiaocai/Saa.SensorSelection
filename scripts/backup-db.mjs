#!/usr/bin/env node
/**
 * 备份 SQLite 数据文件（server/Symtek.Api/App_Data/symtek.db）到 backups/，
 * 保留最近 KEEP 份（默认 10）。
 *
 * 用法：
 *   pnpm run backup:db
 *   KEEP=30 pnpm run backup:db          # 保留 30 份
 *   BACKUP_DIR=/data/symtek-backups pnpm run backup:db
 *
...
const dbPath = join(root, 'server', 'Symtek.Api', 'App_Data', 'symtek.db');
...
const target = join(backupDir, `symtek-${stamp}.db`);
copyFileSync(dbPath, target);
console.log(`已备份到 ${target}`);

const pattern = /^symtek-\d{8}-\d{6}\.db$/;
